...
- 他们提供的SDK是随意使用的，前端打通之后，可以选择传到他们给的的数据接收地址上，但是如果你不购买他们的产品账号，是没有办法查看的；
- 或者只使用它们的api，数据接收到自己家的服务器上，只能查看数据，可视化可以自己写一套看板出来。
- 修改数据接收地址，且send_type选择ajax，后端可以拿到传输的data值，使用base64解码即可！
- 使用base64是因为，在数据传输中，可能有些字符受限，转成base64则大大降低因为字符受限而导致的传输失败，其实数据也不会因为编码变小，还有可能编码之后字符过多，数据变大
